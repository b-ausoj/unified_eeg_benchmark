...
class CSPSVMModel(AbstractModel):
    ...
    def fit(self, X: List[np.ndarray], y: List[np.ndarray], meta: List[Dict]) -> None:
        ...
        # if no channel names, what to do?
    ...
    def _prepare_data(self, X: List[np.ndarray], meta: List[Dict]) -> np.ndarray:
        ...
        X_resampled_filtered = np.array(
            [
                filter_data(
                    data, self.resample_rate, l_freq=8, h_freq=30, verbose=False
                )
                for data in X_resampled
            ]
        )
        X_resampled_filtered = X_resampled_filtered.astype(np.float64)

        return X_resampled_filtered
