...
def decode(X_train, y_train, estimator, n_runs, shuffle_splits, X_test=None,
    ...
    set_preds = {cv_or_eval: pd.DataFrame(),
                 "train": pd.DataFrame()}
    set_preds_rows = {key: [] for key in set_preds.keys()} # added JBU
    set_info = {"feature_importances": pd.DataFrame(),
                "rfpimp_importances": pd.DataFrame(),
                "pca_components": pd.DataFrame(),
                "random_states": pd.DataFrame()}
    set_info_rows = {key: [] for key in set_info.keys()}  # added JBU
    ...
    for run_i in range(n_runs):
        ...
        for key, value in dict_of_dfs.items():
            ...
            set_info_rows[key].append(value) # added JBU

    # added JBU
    for key, rows in set_preds_rows.items():
        set_preds[key] = pd.concat(rows, ignore_index=True)
    for key, rows in set_info_rows.items():
        if (len(rows) > 1):
            set_info[key] = pd.concat(rows, ignore_index=True)
        elif (len(rows) == 1):
            set_info[key] = rows[0]
    ...
    for key in keys:
        ...
        if isinstance(info, list) and len(info) == 0:
            set_info.pop(key)
        elif isinstance(info, pd.DataFrame) and set_info[key].size == 0:
            set_info.pop(key)
