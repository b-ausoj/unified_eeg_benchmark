...
class BENDRBCIModel(nn.Module):
    def __init__(self, num_classes, chunks):
        super().__init__()
        self.chunks = chunks
        ...
        self.linear_probe   = torch.nn.Linear(8704, num_classes)
    ...
    def forward(self, x):
        ...
        if self.chunks is not None and self.chunks == 60:
...
def inference(model, dataloader, device):
    ...
    indices = []
    with torch.no_grad():
        for x, idx in tqdm(dataloader, desc="Testing"):
            logits = model.forward(x.to(device))
...
class BENDRModel(AbstractModel):
    def __init__(
        ...
        self.model = BENDRBCIModel(num_classes=2, chunks=self.chunk_len_s).to(self.device)
