...
from .utils_2 import writer_task, process_one_abnormal, process_one_cli_unm, process_one_epilepsy, LaBraMDataset2, NeuroGPTDataset2, get_channels
...
def make_dataset(X, y, meta, task_name, model_name, chunk_len_s, is_train, use_cache, **kwargs):
    ...
    h5_path = os.path.join(get_config_value("data"), "make_dataset", f"{task_name}_{model_name}_{meta[0]['name'].replace(' ', '_')}_{chunk_len_s}_{True}_{sum(len(obj) for obj in X)}.h5")
