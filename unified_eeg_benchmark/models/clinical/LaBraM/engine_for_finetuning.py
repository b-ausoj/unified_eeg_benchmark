"""
modified and copied from engine_for_finetuning.py
https://github.com/935963004/LaBraM/blob/main/engine_for_finetuning.py
"""
# --------------------------------------------------------
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    log_writer, start_steps, lr_schedule_values, wd_schedule_values,
                    num_training_steps_per_epoch, update_freq: int, ch_names_list, 
                    model_ema: Optional[ModelEma] = None,  is_binary=True):
    ...
    if loss_scaler is None:
        model.zero_grad()
        model.micro_steps = torch.tensor(0)
    else:
        optimizer.zero_grad()
    ...
    for data_loader, ch_names in zip(data_loader_list, ch_names_list):
        ...
        for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            if is_binary:
                ...
                #print(output.shape, output.argmax(dim=-1).shape, targets.squeeze().shape)
                class_acc = (output == targets.squeeze()).float().mean()
