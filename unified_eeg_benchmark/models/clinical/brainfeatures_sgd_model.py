...
class BrainfeaturesSGDModel(AbstractModel):
    def __init__(
        self,
        seed: int = 42,
        ...
        super().__init__('Brainfeatures-SGD')
        self.sgd = SGDClassifier(loss='hinge', random_state=seed, class_weight='balanced')
    ...
    def fit(self, X: List[List[np.ndarray]], y: List[np.ndarray], meta: List[Dict]) -> None:
        ...
        self.sgd.fit(X_prepared, y_prepared)
    ...
    def predict(self, X: List[List[np.ndarray]], meta: List[Dict]) -> np.ndarray:
        ...
        predictions = self.sgd.predict(X_prepared)
