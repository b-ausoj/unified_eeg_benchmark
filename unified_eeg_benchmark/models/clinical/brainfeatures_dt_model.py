...
class BrainfeaturesDTModel(AbstractModel):
    def __init__(
        self,
        seed: int = 42,
        ...
        super().__init__('Brainfeatures-DescisionTree')
        self.dt = DecisionTreeClassifier(random_state=seed, class_weight='balanced')
    ...
    def fit(self, X: List[List[np.ndarray]], y: List[np.ndarray], meta: List[Dict]) -> None:
        ...
        self.dt.fit(X_prepared, y_prepared)
    ...
    def predict(self, X: List[List[np.ndarray]], meta: List[Dict]) -> np.ndarray:
        ...
        return self.dt.predict(X_prepared)

    def _prepare_data(self, X: List[List[np.ndarray]], y, meta: List[Dict]) -> np.ndarray:
