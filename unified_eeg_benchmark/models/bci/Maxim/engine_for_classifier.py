...
def validate_epoch(model, dataloader, device):
    ...
    all_targets = torch.cat(all_targets, dim=0).numpy() # all_targets = torch.cat(all_targets, dim=0)
    all_logits = torch.cat(all_logits, dim=0).numpy() # all_logits = torch.cat(all_logits, dim=0)
